...
public class Dijkstra {
  ...
  public void computeAllEuclideanDistances() {
        // TODO - loop through & call other one
        // loop through all vertices in the map
        // for each vertex's adjacent edges, we'll calculate the distance of that edge
        for (String vertexName : vertexNames.keySet()) {
          Vertex v = vertexNames.get(vertexName);
  ...
  public void doDijkstra(String s) {
        ...
        LinkedList<Vertex> vertexesOfCities = new LinkedList<>();
        ...
        Vertex vert = vertexNames.get(s); // key

        vert.known = true;
        vert.distance = 0.0;

        vertexesOfCities.add(vert);
        while(!vertexesOfCities.isEmpty()) {
            Vertex min = vertexesOfCities.getFirst(); // list method

            for(Vertex x: vertexesOfCities) {
                if(x.distance < min.distance) {
                    min = x;
                }
            }

            vertexesOfCities.remove(min);

            vert = min;
            vert.known = true; //try removing

            for(Edge e: vert.adjacentEdges) {
                tempVertex = e.target;

                if(!tempVertex.known && (vert.distance + e.distance < tempVertex.distance)) {
                    tempVertex.prev = vert;
                    tempVertex.distance = e.distance + vert.distance;
                    vertexesOfCities.add(tempVertex);
                }
            }
        }
  }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    doDijkstra(s);
    ArrayList<Edge> path = new ArrayList<>(); // [A, B, X, Y, Z]
    Vertex end = vertexNames.get(t);
  
    while (end.prev != null) {
      Vertex previous = end.prev;
      Double w = computeEuclideanDistance(end.prev.x, end.prev.y, end.x, end.y);
      Edge e = new Edge(end.prev, end, w);
      path.add(e);
      end = end.prev;
    }
    return path;
